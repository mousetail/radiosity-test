...
fn main() {
    ...
    let mut grid = [[[1 as u16; 16]; 16]; 16];
    for x in 0..16 {
        for y in 0..16 {
            for z in 0..16 {
                let color = image.get_pixel(x as u32, image.height() - 1 - y as u32);
    ...
    voxel_to_mesh(grid, "hallway".to_string()).unwrap();
}
